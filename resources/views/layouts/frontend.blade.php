...
<body class="font-[Plus_Jakarta_Sans] bg-slate-50 text-slate-800 antialiased">
    ...
    <nav id="main-nav" class="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-white/70 border-b border-white/20 transition-all duration-300">
            ...
            <div class="flex justify-between items-center h-20">
                ...
                <a href="/" class="flex items-center gap-3 shrink-0">
                    ...
                    <div class="leading-tight">
                        <h1 class="font-bold text-slate-900">Desa {{ $profil['nama_desa'] ?? 'Gunturmadu' }}</h1>
                        <p class="text-xs text-slate-500">Profil &amp; Informasi Desa</p>
                    </div>
                </a>
                ...
                <button id="menu-toggle" type="button" class="md:hidden relative w-10 h-10 flex items-center justify-center rounded-xl hover:bg-slate-100 transition" aria-label="Buka menu" aria-expanded="false">
                    <svg id="icon-open" class="w-6 h-6 text-slate-700" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path d="M4 6h16M4 12h16M4 18h16"/></svg>
                    <svg id="icon-close" class="w-6 h-6 text-slate-700 hidden" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path d="M6 6l12 12M18 6L6 18"/></svg>
                </button>
            </div>
        </div>
        ...
        <div id="mobile-menu" class="md:hidden hidden border-t border-slate-100 bg-white/95 backdrop-blur-xl">
            <div class="px-6 py-5 flex flex-col gap-1">
                ...
                <a href="{{ route('laporan.create') }}" class="mt-2 block text-center px-5 py-3 rounded-xl bg-emerald-600 text-white text-sm font-bold">Lapor Warga</a>
            </div>
        </div>
    </nav>
    ...
    <footer id="kontak" class="bg-slate-950 text-white">
        <div class="max-w-7xl mx-auto px-6 py-20">
            <div class="grid md:grid-cols-3 gap-12">
                <div>
                    <h3 class="font-bold text-2xl mb-4">Desa {{ $profil['nama_desa'] ?? 'Gunturmadu' }}</h3>
                    <p class="text-slate-400 leading-relaxed">Portal informasi desa untuk mendukung transparansi, pelayanan, dan pengembangan potensi desa.</p>
                    <div class="flex items-center gap-3 mt-6">
                        <a href="{{ $profil['instagram'] ?? '#' }}" target="_blank" class="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-emerald-500/20 hover:border-emerald-500/30 transition">
                        ...
                        <a href="{{ $profil['facebook'] ?? '#' }}" target="_blank" class="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-emerald-500/20 hover:border-emerald-500/30 transition">
                        ...
                        <a href="https://wa.me/{{ $profil['telepon_desa'] ?? '' }}" target="_blank" class="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-emerald-500/20 hover:border-emerald-500/30 transition">
                ...
                <div>
                    ...
                    <div class="space-y-3 text-slate-400 text-sm">
                        <p class="flex items-start gap-2.5"><svg class="w-4 h-4 mt-0.5 text-emerald-400" fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21s-7-6.1-7-11a7 7 0 1114 0c0 4.9-7 11-7 11z"/><circle cx="12" cy="10" r="2.3"/></svg><span>Kantor Desa {{ $profil['nama_desa'] ?? 'Gunturmadu' }}</span></p>
                        <p class="flex items-center gap-2.5"><svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5h4l2 5-2.5 1.5a11 11 0 005 5L13 14l5 2v4a2 2 0 01-2 2C8.6 22 2 15.4 2 7a2 2 0 012-2z"/></svg><span>{{ $profil['telepon_desa'] ?? '(0xx) xxx-xxxx' }}</span></p>
                        <p class="flex items-center gap-2.5"><svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="2"/><path stroke-linecap="round" stroke-linejoin="round" d="M3 7l9 6 9-6"/></svg><span>{{ $profil['email_desa'] ?? 'desa@gunturmadu.desa.id' }}</span></p>
                    </div>
                </div>
            </div>
    ...
    <a href="https://wa.me/{{ $profil['telepon_desa'] }}" target="_blank" aria-label="Hubungi kami via WhatsApp" class="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-emerald-500 hover:bg-emerald-600 shadow-xl shadow-emerald-500/30 flex items-center justify-center text-white transition hover:scale-105">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 20l1.4-4.1A8 8 0 1112 20a8 8 0 01-5-1.7L3 20z"/><path stroke-linecap="round" stroke-linejoin="round" d="M9 9.5c0 3 2.5 5.5 5.5 5.5"/></svg>
    </a>
    @endif
    ...
    <script>
        ...
        window.addEventListener('scroll', () => {
            nav.classList.toggle('shadow-[0_8px_30px_rgba(15,23,42,0.06)]', window.scrollY > 12);
            nav.classList.toggle('bg-white/85', window.scrollY > 12);
            nav.classList.toggle('bg-white/70', window.scrollY <= 12);
        });
